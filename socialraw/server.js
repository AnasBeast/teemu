...
console.log(path.join(__dirname, '../edensound/build/index.html'))
...
var nodemailerFrom = process.env.EMAIL;
    ...
    async function (error, client) {
      ...
      app.post("/getEvents", async function (request, result) {
        ...
        for (let a = 0; a < upcomingEvents.length; a++) {
          upcomingEvents[a].eventDate = getUTCToTZInFormat(
            upcomingEvents[a].eventDate
          );
        }

        const goingEvents = await database
          .collection("events")
          .find({
            "going._id": user._id,
          })
        ...
        for (let a = 0; a < goingEvents.length; a++) {
          goingEvents[a].eventDate = getUTCToTZInFormat(
            goingEvents[a].eventDate
          );
        }
      ...
      app.post("/getStories", async function (request, result) {
        ...
        const myStories = await database
          .collection("stories")
          .find({
            $and: [
              {
        ...
        const myFriendStories = await database
          .collection("stories")
          .find({
            $and: [
              {
                    ...
                    transporter.sendMail(
                      ...
                      function (error, info) {
                        ...
                        result.json({
                          status: "success",
                          message:
                            "Signed up successfully. An email has been sent to verify your account. Once verified, you will be able to login and start using social network.",
                        });
                      }
                    );
      ...
      app.post("/getUser", async function (request, result) {
        ...
        if (user == null) {
          ...
          user.profileViewers = await database
        ...
        database.collection("users").findOne(
          ...
          function (error, user) {
            if (user == null) {
              ...
              if (
                coverPhoto.size > 0 &&
                coverPhoto.type.includes("image")
              ) {
                if (user.coverPhoto != "") {
        ...
        database.collection("users").findOne(
          ...
          function (error, user) {
            if (user == null) {
              ...
              if (
                profileImage.size > 0 &&
                profileImage.type.includes("image")
              ) {
      ...
      app.post("/updateProfile", function (request, result) {
        ...
        var aboutMe = request.fields.about;
                ...
                function (error, user) {
                  if (user == null) {
                    ...
                    for (var a = 0; a < me.friends.length; a++) {
                      if (
                        me.friends[a]._id.toString() == _id.toString() &&
          ...
          function (error, user) {
            if (user == null) {
              ...
              if (type == "ios") {
                ...
                var binaryData = new Buffer(base64Data, "base64").toString(
                  "binary"
                );
      ...
      app.post("/createTicket", async function (request, result) {
        ...
        result.json({
          status: "success",
          message:
            "Ticket has been created. We will respond to your request soon.",
          ticket: ticket.ops[0],
        });
      });
